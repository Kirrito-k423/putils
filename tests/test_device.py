...
from putils.device import TORCH_AVAILABLE
...
class TestDeviceAvailability:
    ...
    def test_is_torch_npu_available_false_no_import(self, mock_torch):
        ...
        from putils.device import is_torch_npu_available
    ...
    def test_is_cuda_available_property(self, mock_torch):
        ...
        from putils.device import is_cuda_available
...
class TestDeviceName:
    ...
    @patch('putils.device.is_cuda_available', True)
    @patch('putils.device.is_npu_available', False)
    ...
    @patch('putils.device.is_cuda_available', False)
    @patch('putils.device.is_npu_available', True)
...
class TestVisibleDevices:
    ...
    def test_get_visible_devices_keyword_cuda(self):
        ...
        from putils.device import get_visible_devices_keyword
...
class TestTorchDevice:
    ...
    @pytest.mark.skipif(not TORCH_AVAILABLE, reason="torch not installed")
    @patch('putils.device.get_device_name')
    @patch('putils.device.torch')
    def test_get_torch_device_success(self, mock_torch, mock_get_device):
        ...
        from putils.device import get_torch_device
...
class TestModuleLevelVariables:
    ...
    def test_npu_available_type(self):
        ...
        from putils.device import is_npu_available
        
        assert isinstance(is_npu_available, bool)

    def test_logger_exists(self):
        """Test logger is defined in module."""
        import putils.device as device_module
