...
class VeomniFlopsCounter:
    ...
    def estimate_flops(self, batch_seqlens, delta_time, **kwargs):
        ...
        promised_flops = 376 # 硬件算力，根据实际情况填写
        mfu = estimated_flops / promised_flops
        return estimated_flops, mfu

if __name__ == "__main__":
    flopcounter = VeomniFlopsCounter(config=AutoConfig.from_pretrained("/mnt/share/weight/Qwen3.5-35B-A3B"))
    flops, mfu = flopcounter.estimate_flops(batch_seqlens=[32768], delta_time=14.0, images_seqlens=[1008])
    print(f"flops: {flops:,} mfu: {mfu *100} %")
